...
void setup()
{
  ...
  background(128);
  min = new Minim(this);
  player = min.loadFile("silly.mp3", 1024);
  started = false;
  score = 0;
  lives = 3;
  time = width * 7;
...
//  debrisImage = loadImage("debris2_blue.png");
  ...
  nebulaImage = loadImage("back.jpg");
...
class Ship extends Sprite
{
  ...
  void drawMe()
  {
    ...
    if (isThrusting)
    {
      imgOn = imgFull.get(imgFull.width, 0, imgFull.width, imgFull.height);    
      image(imgOn, 0, 0);
    } else {
      imgOff = imgFull.get(0, 0, imgFull.width, imgFull.height); 
      image(imgOff, 0, 0);
    }   
    popMatrix();
    imageMode(CORNER);
...
{ 
  ...
  void drawMe()
  {
    ...
    translate(pos[0], pos[1]);
    rotate(angle);
    image(img, 0, 0, img.width, img.width);   
    popMatrix();
    imageMode(CORNER);
  }
